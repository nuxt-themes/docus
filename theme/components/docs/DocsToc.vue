  ...
  <div class="flex flex-col space-y-4">
    ...
    <template v-if="toc?.links?.length">
      ...
      <ul class="-mr-0.5 border-gray-500 pl-3 lg:pr-3">
        <li v-for="link in toc.links" :key="link.text" class="transition-base group min-w-0 border-l-2 transition-colors lg:border-r-2 lg:border-l-0 lg:text-right" :class="activeHeadings.includes(link.id) ? 'border-primary-400 dark:border-primary-600' : 'border-gray-200 dark:border-gray-700'">
          <a
            :href="`#${link.id}`"
