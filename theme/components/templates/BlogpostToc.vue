<template>
  <div class="sticky top-header z-10 text-sm bg-white border-dashed border-b border-gray-100 dark:border-gray-800 backdrop bg-opacity-80 dark:bg-gray-900 dark:bg-opacity-80 px-4">
    <button class="relative z-10 flex items-center w-full py-3 text-sm font-semibold text-gray-900 focus:outline-none dark:text-gray-100" @click="showMobileToc = !showMobileToc">
...
export default {
  ...
  methods: {
    scrollToHeading (e) {
      ...
      setTimeout(() => {
        const offset = document.querySelector(`#${hash}`).offsetTop - parseInt(convertPropToPixels('--blogpost-scroll-margin-block'))
