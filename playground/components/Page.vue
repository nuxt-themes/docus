      ...
      <div v-else>
        <section class="mb-6" :class="{ 'border-b border-gray-200 dark:border-gray-800 pb-6': page.description }">
          <div class="flex items-center justify-between">
            <DComponent v-if="page.icon" :component="page.icon" class="inline-flex mr-3 w-16 h-16 justify-center items-center">
        ...
        <section class="pb-10">
          ...
          >
            <li v-for="link of $docus.nav.en.links[2].children" :key="link.to" class="flex">
              <NuxtLink :to="link.to" class="relative w-full px-6 py-8 bg-gray-50 dark:bg-gray-800 rounded shadow dark:bg-gray-900 hover:shadow-lg">
                <DComponent v-if="link.icon" :component="link.icon" class="w-16 h-16 mb-3" :class="link.iconClass || ''">
