...
<template>
  <div v-if="page" class="py-8 lg:py-16">
    <div class="max-w-4xl px-4 mx-auto sm:px-6 lg:px-8 flex flex-col gap-y-8 items-center justify-center text-center">
      ...
      <h1 v-if="page?.title" class="font-bold text-gray-900 dark:text-white text-5xl">
        {{ page.title }}
      </h1>
      <p v-if="page?.description" class="text-gray-700 dark:text-gray-200 font-medium max-w-5xl">
        {{ page.description }}
      </p>
