...
<template>
  <div class="flex flex-col space-y-4">
    <NuxtLink v-if="prev" :to="prev.path" class="flex items-center overflow-hidden font-semibold group gap-x-3 lg:flex-row-reverse">
    ...
    <template v-if="toc?.links?.length">
      ...
      <ul class="-mr-0.5 border-gray-500 pl-3 lg:pr-3">
        <li
          ...
          :key="link.text"
          class="min-w-0 transition-colors border-l-2 transition-base group lg:border-r-2 lg:border-l-0 lg:text-right"
          :class="activeHeadings.includes(link.id) ? 'border-primary-400 dark:border-primary-600' : 'border-gray-200 dark:border-gray-700'"
        >
          <a
            :href="`#${link.id}`"
            class="block py-1 pl-3 text-sm truncate text-secondary-group-hover lg:pr-3"
            :class="{
              'text-primary-active': activeHeadings.includes(link.id),
    ...
    <NuxtLink v-if="next" :to="next.path" class="flex items-center overflow-hidden font-semibold group gap-x-3 lg:flex-row-reverse">
      <div class="flex-shrink-0 rounded-lg text-secondary-group-hover transition-base">
