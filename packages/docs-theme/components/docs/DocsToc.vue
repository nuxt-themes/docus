...
<template>
  <div class="flex flex-col space-y-4">
    <NuxtLink v-if="prev" :to="prev.path" class="group flex items-center gap-x-3 overflow-hidden font-semibold lg:flex-row-reverse">
    ...
    <template v-if="toc?.links?.length">
      ...
      <ul class="-mr-0.5 border-gray-500 pl-3 lg:pr-3">
        <li
          ...
          :key="link.text"
          class="transition-base group min-w-0 border-l-2 transition-colors lg:border-r-2 lg:border-l-0 lg:text-right"
    ...
    <NuxtLink v-if="next" :to="next.path" class="group flex items-center gap-x-3 overflow-hidden font-semibold lg:flex-row-reverse">
      <div class="text-secondary-group-hover transition-base flex-shrink-0 rounded-lg">
